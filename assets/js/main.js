...
(function ($) {
  ...
  if ($(tpOffcanvasMenu).find(".sub-menu").length != 0) {
    $(tpOffcanvasMenu)
      ...
      .parent()
      .append(
        '<button class="tp-sidemenu-close" aria-label="Open submenu"><svg viewBox="0 0 10 10" aria-hidden="true"><path d="M3 2.5L6.5 5L3 7.5" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></button>',
      );
  }
  ...
  $(".cartmini-close-btn,.tp-offcanvas-overlay").on("click", function () {
    $(".cartmini__area").removeClass(
      "tp-offcanvas-overlay-open cartmini-opened",
    );
    $(".tp-offcanvas-overlay").removeClass("tp-offcanvas-overlay-open");
  });
  ...
  if ($("#tp-header-top-lang-toggle").length) {
    windowOn.on("click", function (e) {
      if (
        $("#tp-header-top-lang-toggle").has(e.target).length ||
        $("#tp-header-top-lang-toggle")[0] === e.target
      ) {
  ...
  if ($("#tp-header-top-currency-toggle").length) {
    windowOn.on("click", function (e) {
      if (
        $("#tp-header-top-currency-toggle").has(e.target).length ||
        $("#tp-header-top-currency-toggle")[0] === e.target
      ) {
        $(".tp-header-top-currency ul").toggleClass(
          "tp-header-top-currency-list-open",
        );
      } else {
        $(".tp-header-top-currency ul").removeClass(
          "tp-header-top-currency-list-open",
        );
      }
    });
  ...
  if ($("#tp-header-login-btn").length) {
    windowOn.on("click", function (e) {
      if (
        $("#tp-header-login-btn").has(e.target).length ||
        $("#tp-header-login-btn")[0] === e.target
      ) {
